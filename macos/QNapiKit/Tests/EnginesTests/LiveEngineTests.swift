...
///
///     QNAPI_LIVE_TESTS=1 swift test --filter LiveEngineTests
///
/// Optional extras:
///   QNAPI_LIVE_MOVIE=/path/to/movie.mkv  — full search+download round-trip
///   QNAPI_OS_API_KEY=...                 — enables the OpenSubtitles tests
private let liveTestsEnabled = ProcessInfo.processInfo.environment["QNAPI_LIVE_TESTS"] != nil
private let liveMoviePath = ProcessInfo.processInfo.environment["QNAPI_LIVE_MOVIE"]
private let osApiKey = ProcessInfo.processInfo.environment["QNAPI_OS_API_KEY"]
...
private let bogusMovie = MovieFileDescriptor(
    url: URL(fileURLWithPath: "/tmp/qnapi-live-bogus.mkv"),
...
private func liveDescriptor() throws -> MovieFileDescriptor {
    let path = try #require(liveMoviePath, "set QNAPI_LIVE_MOVIE for the round-trip tests")
    return try MovieFileDescriptor.hashing(url: URL(fileURLWithPath: path))
}

private func downloadDirectory() -> URL {
    FileManager.default.temporaryDirectory
        .appendingPathComponent("qnapi-live-\(UUID().uuidString)")
}
