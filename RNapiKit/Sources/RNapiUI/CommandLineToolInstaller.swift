...
/// Symlinks the bundled rnapi-cli into /usr/local/bin. Falls back to an
/// admin-privileged AppleScript when the directory isn't user-writable.
@MainActor
enum CommandLineToolInstaller {
    static let linkPath = "/usr/local/bin/rnapi-cli"
    ...
    static var bundledCLI: URL? {
        let helper = Bundle.main.bundleURL
            .appendingPathComponent("Contents/Helpers/rnapi-cli")
    ...
    static func install() -> String? {
        guard let cli = bundledCLI else {
            return String(
                localized: "The command line tool is only bundled in release builds of RNapi.")
        }
