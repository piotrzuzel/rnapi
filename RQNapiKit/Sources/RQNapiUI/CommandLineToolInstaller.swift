...
/// Symlinks the bundled rqnapi-cli into /usr/local/bin. Falls back to an
/// admin-privileged AppleScript when the directory isn't user-writable.
@MainActor
enum CommandLineToolInstaller {
    static let linkPath = "/usr/local/bin/rqnapi-cli"
    ...
    static var bundledCLI: URL? {
        let helper = Bundle.main.bundleURL
            .appendingPathComponent("Contents/Helpers/rqnapi-cli")
    ...
    static func install() -> String? {
        guard let cli = bundledCLI else {
            return String(
                localized: "The command line tool is only bundled in release builds of RQNapi.")
        }
